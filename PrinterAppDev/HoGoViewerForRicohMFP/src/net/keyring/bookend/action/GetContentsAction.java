...
public class GetContentsAction implements ConstReq{
     ...
     */
    public void saveDB_GetContents(ArrayList<ArrayList<NameValuePair>> contentsNewList,
    	...
    	if(!isNull_list(contentsNewList)){
    		// <contentsNew>
    		insertDB(contentsNewList);
    	}
    	if(!isNull_list(contentsUpdateList)){
    		// <contentsUpdate>
    		updateDB(contentsUpdateList);
    	}
    	if(!isNull_list(licenseList)){
    		// <License>
    		updateLicense(licenseList);
    	}
    }
    ...
    private void updateLicense(ArrayList<ArrayList<NameValuePair>> licenseList){
    	...
    	for(ArrayList<NameValuePair> license_list : licenseList){
	    	...
	    	BookBeans book = mContentsDao.setBook(downloadID, ownerPassword, sharedDevice_m, sharedDevice_d, browse_m,
    	...
    	//	更新するレコードが無ければ何もしない
    	if(updateLicense.size() == 0) {
    		return;
    	}
    	
    	if(mContentsDao == null) mContentsDao = new ContentsDao(mCon);
    	mContentsDao.save(updateLicense);
    }
     ...
     */
    private void insertDB(ArrayList<ArrayList<NameValuePair>> contentsNewList){
    	...
    	//	更新するレコードがなければ何もしない
    	if(insertBooks.size() == 0) {
    		return;
    	}
    	
    	if(mContentsDao == null) mContentsDao = new ContentsDao(mCon);
        mContentsDao.save(insertBooks);
    }
     ...
     * @return
     */
    private void updateDB(ArrayList<ArrayList<NameValuePair>> contentsUpdateList){
