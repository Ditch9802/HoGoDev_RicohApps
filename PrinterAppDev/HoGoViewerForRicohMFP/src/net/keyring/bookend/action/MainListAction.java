...
public class MainListAction extends CommonAction implements ConstList {
	...
	private List<BookBeans> mListBook;
	...
	private void setListBook(List<BookBeans> books){
		this.mListBook = books;
	}
	
	public List<BookBeans> getListBook(){
	...
	//** HoGo Custom start **//
	public int listClickAction(Context con, ListView list_group,
			ArrayList<Map<String, Object>> data, DocumentListAdapter adapter, int position) {
		int status = -1;
		BookBeans book = getBook(con, data, adapter, position);
		if (book != null) {
			setBook(book);
	 ...
	 */
	public AlertDialog.Builder setListLongClickDialog(Context con,
			ArrayList<Map<String, Object>> data, DocumentListAdapter adapter,
			int position) {
		AlertDialog.Builder dialog = null;
		// 長押し選択したアイテムのコンテンツ詳細ダイアログ表示（削除ボタン有）
		BookBeans book = getBook(con, data, adapter, position);
	...
	public AlertDialog.Builder setClickButtonDelete(Context con, ArrayList<Map<String, Object>> data, DocumentListAdapter adapter,
			List<Integer> positions){
		AlertDialog.Builder dialog = null;
		String title = "";
		List<BookBeans> listbook = new ArrayList<BookBeans>();
		for(int i = 0; i < positions.size(); i++){
			BookBeans book = getBook(con, data, adapter, positions.get(i));
			listbook.add(book);			
			title = title + setDetailValue(con.getString(R.string.detail_title), book.getTitle() + "<br>");	
			
		}
		
		TextView detail = new TextView(con);
		detail.setText(Html.fromHtml(title));
		...
		//Set ListBook to MainListAction
	...
	public BookBeans getBook(Context con, ArrayList<Map<String, Object>> data, DocumentListAdapter adapter, int position) {
		BookBeans book = null;
		if (data == null) {
			data = getBooksData(con);
		}
	...
	public boolean deleteListBook(Context con) {
		List<BookBeans> listBook = this.mListBook;
		if(listBook != null){
			for(int i = 0; i < listBook.size(); i++){
				BookBeans book = listBook.get(i);
				long rowID = book.getRowId();
				// UpdateLicenseの必要がある場合は行う：sharedDevice:false
				if (book.getSharedDevice_D() != -1) {
					// 共有台数が無制限ではない場合はUpdateLicenseを行う
	...
	public ListView setMainList(ListView list_group, ArrayList<Map<String, Object>> data, DocumentListAdapter adapter) {
		if (data != null && data.size() > 0) {
			adapter.setListData(data); // テンプレートにデータ内容を保持
			list_group.setFocusable(false);
			list_group.setAdapter(adapter);
