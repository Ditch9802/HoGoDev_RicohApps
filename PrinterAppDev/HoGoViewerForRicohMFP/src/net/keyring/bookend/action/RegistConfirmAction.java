	...
	public int checkGetContentsStatus(String status){
		...
		try{
			if(status.equals("11000") || status.equals("11001") || status.equals("11002")){
				...
				// サーバと同期した日時(UTC)を更新
				// #219 PCの時刻ではなく、サーバーと同期した時刻を使用 20140714@kaz
				// delete
				//String last_sync_date = DateUtil.getNowUTC();
				// add
				String last_sync_date = mGetContents.getLastSyncDate();
				//////////
				...
				saveGetContents.saveDB_GetContents(contentsNewList, contentsUpdateList, licenseList);
			}else{
