...
import net.keyring.bookend.activity.HoGoPrintSettingActivity;
 ...
 *
 */
public class HoGoViewTask extends AsyncTask<ViewTaskParam, Integer, String> implements ConstList{
	...
	public HoGoViewTask(Activity a, ViewListener listener){
		this.mActivity = a;
		...
		*/	
			...
			if(fileType == PDF){
				...
				try {
					...
					//strPassword =  new String(userPassword);//DecryptUtil.base16enc(userPassword);
					Logput.d("userPassword = " + DecryptUtil.base16enc(userPassword));
					//	file path of PDF file
					Logput.d("original filePath = " + book.getFile_path());
					
					//	Decrypt PDF 
					FileOutputStream output = mCon.openFileOutput("decrypt_file.pdf", Context.MODE_WORLD_READABLE);
					Utils.decryptPdf(book.getFile_path(), output, userPassword);
					
					//	Decrypted PDF file path
					printFilePath = mCon.getFileStreamPath("decrypt_file.pdf").getAbsolutePath();
					Logput.d("decrypted filePath = " + printFilePath);
	...
	protected void onPostExecute(String result) {
		...
		if(mListener != null){
			...
			Intent intent = new Intent(mActivity, HoGoPrintSettingActivity.class);
			// Set value of File Path
			intent.putExtra("path", printFilePath);
