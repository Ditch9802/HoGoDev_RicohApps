...
//					FileOutputStream output = mCon.openFileOutput(book.getDistributor_name(), Context.MODE_WORLD_READABLE);
...
//					printFilePath = mCon.getFileStreamPath(book.getFile_path()).getAbsolutePath();
	...
	private void decrypt(BookBeans book, boolean isTemporary){
		...
		try {
			...
			String copyPath = mCon.getFilesDir()+ "/" + fileName + ".pdf";
			printFilePath = copyPath;
//			mViewAction.sendAdobeReader(book, copyPath, isTemporary);
		} catch (NullPointerException e) {
			Logput.w(e.getMessage(), e);
			publishProgress(CONTENT_NONE);
